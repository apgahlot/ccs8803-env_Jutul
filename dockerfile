...
RUN julia -e 'using Pkg;Pkg.add("Jutul"); using Jutul'
RUN julia -e 'using Pkg;Pkg.add("JutulDarcy"); using JutulDarcy'
...
RUN julia -e 'using Pkg;Pkg.add("Random")' && \
    ...
    julia -e 'using Pkg; Pkg.add(url="https://github.com/slimgroup/JutulDarcyAD.jl.git#hacksolve")'
